...
function requestGithubLatestRelease(repo) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(
      ...
      {
        headers: {
          'User-Agent': 'ChronoCord-Updater/1.0',
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          try {
            ...
            reject(new Error('Resposta do GitHub inválida.'));
          }
        });
      },
    );
...
function downloadText(url) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(
      u,
      { headers: { 'User-Agent': 'ChronoCord-Updater/1.0' } },
      (res) => {
        let data = '';
        res.setEncoding('utf8');
        res.on('data', (d) => (data += d));
        res.on('end', () => {
          if (res.statusCode !== 200) {
            reject(new Error(`Checksum HTTP ${res.statusCode}`));
            return;
          }
          resolve(data.trim());
        });
...
async function resolveLatest() {
  if (RELEASE_REPO && RELEASE_REPO !== '__RELEASE_REPO__') {
    ...
    if (!exe || !sha) {
      throw new Error('A release mais recente não contém o instalador e o checksum esperados.');
    }
    const checksumText = await downloadText(sha.browser_download_url);
    const checksum = (checksumText.match(/[a-f0-9]{64}/i) || [])[0];
    if (!checksum) throw new Error('Checksum SHA-256 inválido na release.');
    return {
      product: 'ChronoCord',
      version: rel.tag_name.replace(/^chronocord-v/i, '').replace(/^v/i, ''),
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          try {
            ...
            reject(new Error('Manifesto inválido.'));
          }
        });
      },
    );
    ...
    const req = https.get(
      ...
      (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          ...
          download(new URL(res.headers.location, u).href, dest, onProgress).then(resolve, reject);
          return;
        }
        if (res.statusCode !== 200) {
          file.close();
...
function waitPid(pid) {
  return new Promise((resolve) => {
    ...
    const check = () => {
      try {
        process.kill(pid, 0);
        if (Date.now() - started > 20000) return resolve();
...
async function performUpdate(manifest) {
  ...
  if (actual.toLowerCase() !== String(manifest.sha256 || '').toLowerCase()) {
    fs.rmSync(dest, { force: true });
    throw new Error('A verificação de integridade falhou. O instalador foi descartado.');
  }
  ...
  const child = spawn(dest, ['--updated'], { detached: true, stdio: 'ignore', windowsHide: false });
  child.unref();
...
ipcMain.handle('update-now', async () => {
  try {
    ...
    send('error', { text: e.message || 'Não foi possível atualizar.' });
    return { ok: false, error: e.message };
  }
});

ipcMain.handle('later', () => {
  app.quit();
  return true;
});
...
app.whenReady().then(async () => {
  ...
  try {
    const m = await resolveLatest();
    if (!m || m.product !== 'ChronoCord' || !m.version || !m.url || !m.sha256) {
      throw new Error('Atualização inválida.');
    }
    if (cmp(m.version, currentVersion) <= 0) {
      app.quit();
      return;
    }
    win.webContents.once('did-finish-load', () => {
      send('available', {
        version: m.version,
        title: m.title || `ChronoCord ${m.version}`,
        notes: m.notes || '',
