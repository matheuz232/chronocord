export function chronocordProductFeatures() {
  return {
    ...
    transform(code, id) {
      ...
      if (output.includes(fmtMarker) && !output.includes("localStorage.setItem('chronocord.jukebox.queueVisible'")) {
      ...
      if (output.includes(nowPlayingCard) && !output.includes('cc-jukebox-art-card')) {
        output = output.replace(nowPlayingCard, nowPlayingCardReplacement);
        changed = true;
      }

      const inlineQueueMarker = `          {queue.length > 0 && (\n            <div style={{ marginBottom: 14 }}>`;
      if (output.includes(inlineQueueMarker)) {
        output = output.replace(inlineQueueMarker, `          {queue.length > 0 && (\n            <div className="cc-jukebox-inline-queue" style={{ marginBottom: 14, display: showJukeboxQueue ? 'none' : 'block' }}>`);
        changed = true;
      }
