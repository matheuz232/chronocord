...
const LEGACY_ROUTE_MAP = {
  ...
  permissoes: 'messaging.content',
  notificacoes: 'notifications.overview',
  voz: 'voice.voice',
  camera: 'voice.camera',
  transmissao: 'voice.stream',
  sons: 'voice.sounds',
  soundboard: 'voice.soundboard',
  avancado: 'developer',
  acessibilidade: 'accessibility.readability',
  sistema: 'system.general',
  idioma: 'language',
  aparencia: 'appearance.theme',
};

function routeGroup(route) {
  if (route === 'profile.edit') return 'profile';
...
function routeTitle(route) {
  ...
  const groupId = routeGroup(route);
...
export default function SettingsCenter({ user, profile, T, themeColor, onClose, onLogout, legacy = {} }) {
  ...
  useEffect(() => {
    ...
    setSettings(loaded);
    const next = LEGACY_ROUTE_MAP[legacy?.settingsTab] || loaded.ui?.lastSettingsRoute || 'account.info';
    setRoute(next);
  }, [userId]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => saveSettings(userId, settings), 200);
    return () => clearTimeout(timer);
  }, [settings, userId]);

  useEffect(() => () => {
    try { saveSettings(userId, settingsRef.current); } catch {}
  }, [userId]);

  const patch = useCallback((path, value) => {
    setSettings((current) => patchSettingsPath(current, path, typeof value === 'function' ? value(current) : value));
  }, []);

  const resetSubtree = useCallback((path) => {
    setSettings((current) => resetSettingsSubtree(current, path));
  }, []);

  const navigate = useCallback((nextRoute) => {
    if (!nextRoute) return;
    setRoute(nextRoute);
    setSettings((current) => patchSettingsPath(current, 'ui.lastSettingsRoute', nextRoute));
    setQuery('');
  }, []);
  ...
  useEffect(() => {
    const frame = requestAnimationFrame(() => {
      ...
      if (!host) return;
      const target = [...host.querySelectorAll('[data-settings-route]')].find((node) => node.dataset.settingsRoute === route);
      target?.scrollIntoView?.({ behavior: 'smooth', block: 'start' });
    });
    return () => cancelAnimationFrame(frame);
  }, [route]);

  const legacyWithNavigation = useMemo(() => ({
    ...legacy,
    openProfileEditor: () => navigate('profile.edit'),
  }), [legacy, navigate]);
  ...
  const page = renderPage(route, {
    ...
    themeColor,
    navigate,
    legacy: legacyWithNavigation,
  });

  const cssVars = {
    '--cc-bg-0': T?.bg0 || '#0E0C18',
    '--cc-bg-1': T?.bg1 || '#151228',
    '--cc-bg-2': T?.bg2 || '#1B1832',
    '--cc-bg-3': T?.bg3 || '#211D3D',
    '--cc-bg-4': T?.bg4 || '#262146',
    '--cc-border': T?.border || '#332C57',
    '--cc-text-main': T?.textMain || '#EFEBFB',
    '--cc-text-dim': T?.textDim || '#9A93B8',
    '--cc-text-faint': T?.textFaint || '#6A6390',
    '--cc-accent': themeColor || '#9B4DFF',
    '--cc-accent-text': T?.text || '#fff',
  };
    ...
    <div className="cc-settings-root" style={cssVars} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <div className="cc-settings-window" role="dialog" aria-modal="true" aria-label="Configurações do ChronoCord">
        ...
        <main className="cc-settings-main">
          <div className="cc-settings-topbar">
            <div><strong>{routeTitle(route)}</strong><span>ChronoCord</span></div>
            <button type="button" className="cc-settings-close" aria-label="Fechar configurações" onClick={close}>×</button>
          </div>
          <div className="cc-settings-scroll" ref={scrollRef}>
            <div className="cc-settings-content">{page}</div>
          </div>
        </main>
      </div>
    </div>
